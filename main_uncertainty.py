...
if __name__ == "__main__":    
    ...
    parser.add_argument("--save_path", type=str, default="result_sof")
    parser.add_argument("--num_adj", type=int, default=3)
    parser.add_argument("--exp", type=str, default="exp_sof")
    ...
    if not os.path.isfile(f"{save_path}/n_trials{num_trials}_adj{num_adj}.csv"):
        ...
        for t in range(num_trials):
            if data_folder in ['ROSMAP', 'LGG', 'SOF']:
                num_class = 2
            if data_folder in ['BRCA', 'KIPAN']:
                num_class = 5
            if data_folder in ["CUB", "Handwritten"]:
                num_class = 10
